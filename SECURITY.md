...
<https://github.com/voicetel/voiceml-php-sdk/security/advisories/new>).
...
- Do not log the configured `apiKey` / `authToken` or the
...
- The SDK uses Guzzle 7 with PHP's default OpenSSL TLS posture
  (TLS 1.2+). If you inject a custom Guzzle client, you are
...
Out of scope: vulnerabilities in the published
`voicetel/voiceml` Composer package caused by a forked / vendored

...
/** `GET /v2/Voice/Countries/{IsoCountry}` body. */
final class PricingVoiceCountryV2 implements Model
{
    /**
     * @param list<PricingOutboundPrefixPriceWithOrigin> $outboundPrefixPrices
    ...
    public static function fromArray(array $data): self
    {
        ...
        foreach ((array) ($data['outbound_prefix_prices'] ?? []) as $row) {
            if (is_array($row)) {
                $outbound[] = PricingOutboundPrefixPriceWithOrigin::fromArray($row);
            }
        }
        $inbound = [];

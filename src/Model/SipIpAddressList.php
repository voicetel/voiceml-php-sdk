...
/** Paginated /IpAddresses response. */
final class SipIpAddressList implements Model
{
    /** @param list<SipIpAddress> $ipAddresses */
    ...
    {
        $items = [];
        foreach ((array) ($data['ip_addresses'] ?? []) as $row) {
            if (is_array($row)) $items[] = SipIpAddress::fromArray($row);
        }
        return new self(
            ipAddresses: $items,
